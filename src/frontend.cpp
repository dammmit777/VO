...
bool Frontend::Track()
{
    ...
    if (tracking_inliers_ > num_features_tracking_)
        status_ = FrontendStatus::TRACKING_GOOD; //tracking good
    else if (tracking_inliers_ > num_features_tracking_bad_)
        status_ = FrontendStatus::TRACKING_BAD; //tracking bad
    else
        status_ = FrontendStatus::LOST; //lost
...
bool Frontend::InsertKeyframe()
{
    if (tracking_inliers_ >= num_features_needed_for_keyframe_)
        return false; //仍然有足够的特征，不需要插入关键帧
...
int Frontend::TrackLastFrame()
{
    ...
    Mat error;
    cv::calcOpticalFlowPyrLK(last_frame_->left_img_, current_frame_->left_img_, kps_last,
